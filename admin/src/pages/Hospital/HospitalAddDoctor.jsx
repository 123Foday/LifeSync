...
const HospitalAddDoctor = () => {
  ...
  const { backendUrl, hToken, getDoctors, profileData, getProfileData } = useContext(HospitalContext)
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      // include hospital id so the doctor is created under this hospital
      formData.append('hospitalId', profileData?._id || '')

      const {data} = await axios.post(backendUrl + '/api/hospital/create-doctor', formData, {headers:{ Authorization: `Bearer ${hToken}` } })
      ...
      <div className='bg-white dark:bg-zinc-900 px-8 py-8 border dark:border-zinc-800 rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll no-scrollbar'>
        ...
        <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-600 dark:text-gray-300'>
          ...
          <div className='w-full lg:flex-1 flex flex-col gap-4'>
            ...
            <div className='flex-1 flex flex-col gap-1'>
              <p>Hospital</p>
              <input readOnly value={profileData? profileData.name : 'Your hospital'} className='border dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950 rounded px-3 py-2 outline-none transition-all dark:text-gray-400' />
            </div>
        ...
        <div>
              <p className='mt-6 mb-2'>About Doctor</p>
