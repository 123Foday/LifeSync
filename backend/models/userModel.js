import mongoose from "mongoose";

const userSchema = new mongoose.Schema(
  {
    name: {
      type: String,
      required: function () {
        // Name is always required, but will come from SSO provider if not local
        return true;
      },
    },
    email: {
      type: String,
      required: true,
      unique: true,
      lowercase: true,
      trim: true,
    },
    password: {
      type: String,
      required: function () {
        // Password only required for local authentication
        return this.authProvider === "local";
      },
    },

    // SSO Provider IDs
    googleId: {
      type: String,
      sparse: true, // Allows multiple null values
      unique: true,
    },
    microsoftId: {
      type: String,
      sparse: true,
      unique: true,
    },
    appleId: {
      type: String,
      sparse: true,
      unique: true,
    },

    // Profile Information
    image: {
      type: String,
      default:
        "data:image/png;base64,",
    },
    address: {
      type: Object,
      default: { line1: "", line2: "" },
    },
    gender: {
      type: String,
      default: "Not Selected",
    },
    dob: {
      type: String,
      default: "Not Selected",
    },
    phone: {
      type: String,
      default: "000000000",
    },

    // Authentication Provider
    authProvider: {
      type: String,
      enum: ["local", "google", "microsoft", "apple"],
      default: "local",
    },

    // Account status
    isEmailVerified: {
      type: Boolean,
      default: function () {
        // SSO emails are pre-verified
        return this.authProvider !== "local";
      },
    },
  },
  {
    timestamps: true, // Adds createdAt and updatedAt
  }
);

// No need for explicit indexes - unique: true and sparse: true already create indexes
// The 'unique' option automatically creates an index

const userModel = mongoose.models.user || mongoose.model("user", userSchema);

export default userModel;
