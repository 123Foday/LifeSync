...
const Navbar = () => {
  ...
  return (
    <div className={`sticky top-0 z-50 bg-white transition-shadow ${hasShadow ? 'shadow-md' : ''}`}>
      <div className="flex items-center justify-between text-xs sm:text-sm md:text-sm lg:text-base py-2.5 sm:py-3 md:py-3 lg:py-4 border-b border-b-gray-400 px-2 sm:px-3 md:px-4 lg:px-6">
        <img
          onClick={() => navigate("/")}
        ...
        <div className="hidden md:block relative">
          <ul ref={navRef} className="flex items-center gap-2 md:gap-3 lg:gap-4 xl:gap-5 font-medium pb-1 text-xs md:text-sm lg:text-base whitespace-nowrap">
            {navItems.map((item, index) => (
              ...
              >
                <NavLink
                  ...
                  className={({ isActive: isNavActive }) => `py-1 transition-colors duration-100 whitespace-nowrap ${isNavActive ? 'text-[#5f6FFF]' : 'text-gray-500 hover:text-[#5f6FFF]'}`}
                >
                  {item.label}
                </NavLink>
              </li>
        ...
        <div className="hidden md:flex items-center relative">
          <input
            ...
            className="px-2 md:px-3 py-1.5 md:py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs md:text-sm w-32 md:w-40 lg:w-48 xl:w-64"
          />
        ...
        <div className="flex items-center gap-2 md:gap-3 lg:gap-4">
          {token && userData ? (
            <div className="flex items-center gap-1.5 md:gap-2 cursor-pointer group relative">
              <img className="w-7 md:w-8 rounded-full flex-shrink-0" src={userData.image} alt="User" />
              <img className="w-2 md:w-2.5 flex-shrink-0" src={assets.dropdown_icon} alt="Dropdown" />
              <div className="absolute top-10 right-0 pt-14 text-sm md:text-base font-medium text-gray-600 z-20 hidden group-hover:block">
                <div className="w-40 md:w-44 max-w-[90vw] bg-stone-100 rounded flex flex-col gap-3 md:gap-4 p-3 md:p-4">
                  <p onClick={() => navigate("/my-profile")} className="hover:text-black cursor-pointer text-xs md:text-sm">My Profile</p>
                  <p onClick={() => navigate("/my-appointments")} className="hover:text-black cursor-pointer text-xs md:text-sm">My Appointment</p>
                  <p onClick={logout} className="hover:text-black cursor-pointer text-xs md:text-sm">Logout</p>
                </div>
              </div>
            </div>
          ) : (
          ...
          <div className='flex items-center gap-2'>
            <button aria-label="Open search" onClick={() => { setShowSearchMobile(true); setShowMenu(false); }} className="md:hidden p-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-gray-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="7" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
            </button>

            <button aria-label="Open menu" aria-expanded={showMenu} aria-controls="mobile-menu" onClick={() => { setShowMenu(true); setShowSearchMobile(false); }} className="md:hidden p-2">
              <img className="w-6" src={assets.menu_icon} alt="Menu" />
            </button>
          </div> 

          {/* Mobile Menu */}
          <div className={`fixed inset-0 z-40 md:hidden ${showMenu ? '' : 'pointer-events-none'}`} aria-hidden={!showMenu}>
            {/* Backdrop */}
            <div onClick={() => setShowMenu(false)} className={`absolute inset-0 bg-black/40 transition-opacity ${showMenu ? 'opacity-100' : 'opacity-0'}`} aria-hidden="true" />

            {/* Panel */}
            <aside ref={menuRef} id="mobile-menu" role="dialog" aria-modal={showMenu} aria-hidden={!showMenu} className={`absolute right-0 top-0 bottom-0 w-3/4 sm:w-1/2 bg-white transform ${showMenu ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 overflow-auto`}>
              <div className="flex items-center justify-between px-5 py-6">
                <img className="w-32 md:w-36" src={assets.logo} alt="Logo" />
                <button className="w-8 h-8 rounded p-1" onClick={() => setShowMenu(false)} aria-label="Close menu">
                  <img className="w-7" src={assets.cross_icon} alt="Close" />
                </button>
              </div>
              <ul className="flex flex-col items-center gap-2 mt-5 px-5 text-base sm:text-lg font-medium">
        ...
        {showSearchMobile && (
          <div ref={mobileSearchPanelRef} role="dialog" aria-modal={showSearchMobile} aria-hidden={!showSearchMobile} className="fixed inset-0 z-50 bg-white p-4 overflow-auto md:hidden">
            <div className="flex items-center gap-2 mb-4">
              <button onClick={() => setShowSearchMobile(false)} aria-label="Close search" className="p-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-gray-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
              <input
                ref={mobileSearchRef}
                type="text"
            ...
            {showSearchResults && searchResults.length > 0 && (
              <div className="flex flex-col gap-2">
                {searchResults.map((item, idx) => (
                  <div
                    key={idx}
                    onClick={() => { handleSearchSelect(item, item.availableSlots ? 'doctor' : 'hospital'); setShowSearchMobile(false); }}
                    className="px-4 py-2 hover:bg-gray-100 cursor-pointer border-b border-gray-200 last:border-b-0"
                  >
                    <p className="font-medium text-sm">{item.name}</p>
                    <p className="text-xs text-gray-500">{item.speciality || item.address}</p>
